...
### 17.1 Prioritized Backlog (document v1.6)

Every OI raised through v1.5 is now resolved (OI-1 through OI-12), plus four more closed during this audit (OI-13 through OI-16). What's left, ranked:

| Priority | Item | Why this rank | Rough effort |
|---|---|---|---|
| **1 — High** | §9.10 Vault Path Blacklist (`BLK`) — build `FR-BLK-1..5`/`NFR-BLK-1..3` | The only remaining item that's a real, operator-requested, user-facing capability rather than test/doc hygiene. Fully specified and scoped already (including the `propose_edit` interaction resolved by OI-11) — nothing left to decide, only to build. | Medium: touches `server.py` (indexer + `read_note` + `propose_edit`, all via the shared `_resolve_in_vault`/`build_index` fallback per NFR-BLK-3), plus `compose.yaml`/README for the new `VAULT_BLACKLIST` env var. |
| **2 — Medium** | OI-17 — `push-sync.sh` test coverage | Real silent-failure risk in a component that decides whether a note or proposal actually reaches the vault correctly; currently zero coverage. Ranked below BLK only because it's hardening existing behavior, not new capability. | Medium-High: needs a shell-testing approach this repo doesn't have precedent for (subprocess harness against a scratch git remote, or adopt `bats`). |
| **3 — Medium** | OI-18 — `FR-SRCH-2`/`FR-SRCH-3` exact-shape tests | Locks in the already-fixed 5-vs-10 discrepancy (RISK-1) against silent regression; pure Python, no new test infrastructure needed. | Low: two focused tests in `tests/test_server.py`. |
| **4 — Low** | OI-19 — direct `_make_diff` index-line assertion | Marginal value over the existing behavioral test; only worth doing opportunistically alongside other `propose_edit` work. | Trivial. |
| — (no action) | Remaining "not unit-testable in isolation" NFRs (`NFR-COM-2/3/5`, `NFR-OVW-1/3`, `NFR-SRCH-3`, `NFR-READ-2/3`, `NFR-NOTE-1/3`, `NFR-PROP-1/4/5/6/7`, `NFR-AUTH-1/3/4/7`) | Genuine architectural/deployment/protocol facts, not gaps — asserting "runs on Kubernetes" or "delegates identity to Dex" isn't something a unit test proves. Listed here only so a future audit doesn't re-flag them as findings. | N/A |
