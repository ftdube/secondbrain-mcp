...
| FR-READ-4 | No file-size limit, truncation, or pagination is applied; the entire file is returned in one response. | Must (as implemented) | Implemented — see NFR-READ-2 for the risk this carries |
...
| NFR-READ-2 | **Gap:** FR-READ-4's lack of a size cap means one very large note can consume a disproportionate share of a session's token budget. No mitigation is implemented. | Should | Not implemented — see OI-2 |
...
| ID | Requirement | Priority | Status |
|---|---|---|---|
| NFR-SEC-1 | The system SHALL NOT bulk-replicate vault content off the self-hosted environment; only per-call tool results (bounded by FR-SRCH-2, and *not* bounded for FR-READ-4 — see NFR-READ-2) leave the environment boundary. | Must | Implemented (core privacy claim, §2) |
...
| RISK-7 | Concurrent `apply-proposals` runs are not guarded; two overlapping invocations (e.g. a cron job and a manual run) could interleave `check()`/`apply()` steps across processes. | Low (single operator) | Medium | Not mitigated — documented operational constraint (run serially); candidate future lockfile — see OI-3 |
| RISK-8 | The authentication middleware (`BearerAuthMiddleware`, JWT validation, public-path allowlist) has no automated test coverage. | N/A (verified fact, not probabilistic) | High if a future refactor silently weakens the auth gate | Not mitigated — see OI-4 |
